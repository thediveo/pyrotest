...
// HistoryEmptyBucketsMatcher matches the empty buckets including the implicit
// “+Inf” empty bucket.
...
// HistorySomeFilledBucketsMatcher matches any non-empty buckets including the
// implicit “+Inf” bucket.
...
var (
	_ (MetricPropertyMatcher) = (*HistorySomeFilledBucketsMatcher)(nil)
	_ (metricPropertyMatcher) = (*HistorySomeFilledBucketsMatcher)(nil)
	_ (format.GomegaStringer) = (*HistorySomeFilledBucketsMatcher)(nil)
)

func (m *HistorySomeFilledBucketsMatcher) GomegaString() string {
	return "buckets: <non-empty>"
}

func (m *HistorySomeFilledBucketsMatcher) matchProperty(metric *prommodel.Metric) (bool, error) {
	if metric.Histogram == nil {
		return false, nil
	}
	if metric.Histogram.GetSampleCount() != 0 {
